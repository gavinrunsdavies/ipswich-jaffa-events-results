...
add_action('template_redirect', function () {
    ...
    if ($pdfFlag) {
        ...
        if ($meeting) {
            $_GET['meetingId'] = $meeting;
        }
        if ($race) {
            $_GET['raceId'] = $race;
        }

        // No assets to enqueue here — this route sends raw binary PDF
        // bytes with a Content-Type: application/pdf header, so there's
        // nothing for DataTables (or any HTML/CSS/JS) to attach to.
